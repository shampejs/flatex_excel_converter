...
class Transaction:
    def __init__(self, data, date, name, amount, kind_of_transaction, course, end_amount, winning, kest):
        self.data = data
        self.date = date
        self.name = name
    ...
    def get_excel_format(self):
        return [self.date, self.name, self.kind_of_transaction, self.course, self.amount, self.course * self.amount,
                "", "", "", self.kest, self.end_amount, self.winning]
